...
class TorqueRunner:
    ...
    def _run_and_evaluate(
        ...
        try:
            ...
            # Step 2.5: Apply preprocessing if specified
            scaler = None
            if preprocessing and preprocessing != "none":
                if not PREPROCESSING_AVAILABLE:
                    print(f"  ⚠ Preprocessing '{preprocessing}' requested but sklearn.preprocessing not available")
                else:
                    print(f"Step 2.5: Applying preprocessing ({preprocessing})...")
                    if preprocessing == "standard":
                        ...
                        print(f"  ⚠ Unknown preprocessing: {preprocessing}, skipping")
            ...
            results["preprocessing"] = preprocessing if preprocessing else "none"

            # Step 2.6: Apply SMOTE to training data only (if requested)
            if use_smote:
                if not SMOTE_AVAILABLE:
                    print("  ⚠ SMOTE requested but imbalanced-learn not installed. Run: pip install imbalanced-learn")
                else:
                    try:
