"""
Pyparsing-based DSL parser.

Parses DSL strings into AST nodes.
"""
...
class DSLParser:
    """Parser for Torque DSL using pyparsing."""
    ...
    def _make_literal_node(self, tokens):
        """Create a dict-based LiteralNode from parsed tokens."""
        value = tokens[0]
    ...
    def _make_call_node(self, tokens):
        """Create a dict-based CallNode from parsed tokens."""
    ...
    def parse(self, dsl_string: str) -> dict:
        ...
        if not result:
            raise ValueError(f"Failed to parse DSL: {dsl_string}")
...
def parse_dsl(dsl_string: str) -> dict:
    """Alias for parse_dsl_to_ast (backward compatibility)."""
    return parse_dsl_to_ast(dsl_string)
...
def parse_dsl_to_ast(dsl_string: str) -> dict:
    """
    Parse a DSL string into a dict-based AST.
